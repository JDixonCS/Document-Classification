...
```{r Chapter 3 EC 1}
# Problematic Code
# ggplot(table1, aes(x = precision, y = test_size)) + 
#   geom_point(size = 2, colour = "grey30") + 
#   geom_abline(
#     aes(intercept = a1, slope = a2, colour = -dist), 
#     data = table1(models, rank(dist) <= 10)
#   )
# Error in x[!nas] : object of type 'closure' is not subsettable1
```
<br>
...
ggplot(data = table1) +
  geom_count(mapping = aes(x = precision, y = test_size, alpha = classifier))
```
<br>
```{r Chapter 3 Graph 15 Sampling}
ggplot(data = table1) +
  geom_count(mapping = aes(x = precision, y = test_size, alpha = sampling))
```
<br>
```{r Chapter 3 Graph 15 Technique}
ggplot(data = table1) +
  geom_count(mapping = aes(x = precision, y = test_size, alpha = technique))
```
<br>
...
ggplot(data = table1) +
  geom_bar(mapping = aes(x = test_size))
```
<br>
...
ggplot(data = smaller, mapping = aes(x = test_size)) +
  geom_histogram(binwidth = 0.1)
```
<br>
```{r Chapter 7 EDA Graph 4 Classifier ~ Precision}
ggplot(data = smaller, mapping = aes(x = Precision, colour = test_size)) +
...
ggplot(data = table1, mapping = aes(x = x, y = y)) + 
  geom_point()
```
<br>
...
ggplot(data = table1, mapping = aes(x = precision)) + 
...
ggplot(data = table1, mapping = aes(x = recall)) + 
  geom_freqpoly(mapping = aes(colour = classifier), binwidth = 500)
```
<br>
...
ggplot(data = table1, mapping = aes(x = precision, y = ..recall..)) + 
  geom_freqpoly(mapping = aes(colour = classifier), binwidth = 500)
```
<br>
...
ggplot(data = table1, mapping = aes(x = classifier, y = precision)) +
  geom_boxplot()
```
<br>
```{r Chapter 7 EDA Graph 17 Sampling}
ggplot(data = table1, mapping = aes(x = sampling, y = precision)) +
  geom_boxplot()
```
<br>
```{r Chapter 7 EDA Graph 17 Technique}
ggplot(data = table1, mapping = aes(x = technique, y = precision)) +
  geom_boxplot()
```
<br>
```{r Chapter 7 EDA Graph 18 Classifier}
ggplot(data = table1, mapping = aes(x = classifier, y = test_size)) +
  geom_boxplot()
```
<br>
```{r Chapter 7 EDA Graph 18 Sampling}
ggplot(data = table1, mapping = aes(x = sampling, y = test_size)) +
  geom_boxplot()
```
<br>
```{r Chapter 7 EDA Graph 18 Technique}
ggplot(data = table1, mapping = aes(x = technique, y = test_size)) +
  geom_boxplot()
```
<br>
...
ggplot(data = table1) +
  geom_boxplot(mapping = aes(x = reorder(classifier, recall, FUN = median), y = test_size))
```
<br>
...
ggplot(data = table1) +
  geom_boxplot(mapping = aes(x = reorder(sampling, recall, FUN = median), y = test_size))
```
<br>
...
ggplot(data = table1) +
  geom_boxplot(mapping = aes(x = reorder(technique, recall, FUN = median), y = test_size))
```
<br>
...
ggplot(data = table1) + 
  geom_point(mapping = aes(x = Precision, y = test_size), alpha = 50 / 100)
```
<br>
```{r Chapter 7 EDA Graph 24}
ggplot(data = smaller) +
  geom_bin2d(mapping = aes(x = Precision, y = test_size))
```
<br>
```{r Load Hexbin}
...
ggplot(data = smaller) +
  geom_hex(mapping = aes(x = Precision, y = test_size))
```
<br>
```{r Chapter 7 EDA Graph 26}
ggplot(data = smaller, mapping = aes(x = Precision, y = test_size)) + 
...
mod <- lm(log(Recall) ~ log(Precision), data = table1)
...
models
ggplot(table1, aes(x=precision, y=recall)) + 
...
table1_dist <- function(a1, a2) {
  measure_distance(c(a1, a2),table1)
}
# test2 <- c(precision[1],recall[2])
# test2
# measure_distance(c(7.5, 1), table1)

models <- models %>% 
  mutate(dist = purrr::map2_dbl(a1, a2, table1_dist))
models
```
#> # A tibble: 250 x 3
#>       a1      a2  dist
...
```
# best <- optim(c(0, 0), measure_distance, data = table1)
# best$par

# ggplot(table1, aes(x, y)) + 
#   geom_point(size = 2, colour = "grey30") + 
#   geom_abline(intercept = best$par[1], slope = best$par[2])
```{r Chapter 23 Plot 7}
table1_mod <- lm(y ~ x, data = table1)
coef(table1_mod)
...
ggplot(table1, aes(resid)) + 
  geom_freqpoly(binwidth = 0.5)
```
<br>
```{r Chapter 23 Plot 10}
ggplot(table1, aes(x, resid)) + 
  geom_ref_line(h = 0) +
  geom_point() 
```
<br>
...
mod1 <- lm(y ~ x + classifier, data = table1)
mod2 <- lm(y ~ x * classifier, data = table1)
